...
export default function ClosingContent({ mode }: { mode: "handoff" | "dayclose" }) {
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-lg mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-base font-semibold text-gray-900">{mode === "dayclose" ? "ปิดยอดสิ้นวัน" : "นับเงินเปลี่ยนกะ"}</p>
      ...
      {loading ? (
        ...
        <div className="max-w-lg mx-auto px-4 pt-4 flex flex-col gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col gap-3">
            <div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500">ยอดรับต่อจากกะก่อน</span>
                <span className="text-sm font-bold text-gray-900">{formatCurrency(data.openingFloat, country)}</span>
              </div>
              <p className="text-[11px] text-gray-400 mt-1">
                {data.openingIsNewDay
                  ? "เริ่มวันใหม่ — ยอดนี้คือเงินในเก๊ะที่เหลือไว้หลังปิดร้าน"
                  : `นับไว้โดย ${data.openingFrom?.closedByName ?? "-"}${data.openingFrom?.shiftName ? ` (${data.openingFrom.shiftName})` : ""} เมื่อ ${data.openingFrom ? new Date(data.openingFrom.createdAt).toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit" }) + " น." : "-"} — เข้ากะไม่ต้องนับใหม่ ใช้ยอดนี้ต่อได้เลย`}
              </p>
            </div>
          ...
          {data && mode === "handoff" && (
            <div className="bg-white rounded-2xl border border-gray-100 p-4">
              <p className="text-xs text-gray-400">
                นับเพื่อส่งต่อกะเท่านั้น — เงินทั้งหมดอยู่ในเก๊ะเหมือนเดิม กะถัดไปทำงานต่อบนยอดนี้
              </p>
              <p className="text-[11px] text-gray-400 mt-2 pt-2 border-t border-gray-50">
                จะปิดร้านเลิกวันนี้เลย? <Link href={`/dashboard/cash-closing${storeId ? `?storeId=${storeId}` : ""}`} className="font-bold text-gray-600 underline">ไปหน้าปิดยอดสิ้นวัน</Link>
              </p>
            </div>
          )}
